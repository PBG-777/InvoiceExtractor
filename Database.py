...
class Database:
    ...
    def create_Table(self):
        ...
        cur.execute('CREATE TABLE IF NOT EXISTS rechnungen (Firmenname TEXT, Datum TEXT, Iban TEXT, Gesamtbetrag TEXT, '
                    'Rechnungsnummer VARCHAR(10) PRIMARY KEY,  Zahlungsfrist TEXT, Telefonnummer TEXT)')
    ...
    def set_Data(self):
        ...
        for element in var.pdf_text_extraction():
            # Schleife durchläuft die Elemente der Liste, die Elemente enthalten die Dictionaries
            rechnungs_keys = self.get_rechnungen_keys()
            if str(element['RECHNUNGSNUMMER']) not in rechnungs_keys:
                cur.execute('INSERT INTO rechnungen (FIRMENNAME,DATUM,IBAN,GESAMTBETRAG,RECHNUNGSNUMMER,'
                            'ZAHLUNGSFRIST,TELEFONNUMMER) VALUES ( %s, %s, %s, %s, %s, %s, %s) ', (
